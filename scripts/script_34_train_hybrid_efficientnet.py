...
print('\nBuilding Hybrid EfficientNetB0 model (Quality-as-Feature)...')
image_input = tf.keras.Input(shape=(224, 224, 3), name='image_input')
efficientnet_base = EfficientNetB0(weights='imagenet', include_top=False, input_tensor=image_input)
...
print(f'\nHybrid EfficientNetB0 Test Accuracy: {test_accuracy:.4f}')
hybrid_model.save('hybrid_efficientnet.h5')
print('Hybrid EfficientNetB0 model saved as hybrid_efficientnet.h5')
combined_history = {}
for key in history_phase1.history:
    combined_history[key] = history_phase1.history[key] + history_phase2.history[key]
for key in combined_history:
    combined_history[key] = [float(v) for v in combined_history[key]]
with open('results/hybrid_efficientnet_history.json', 'w') as f:
    json.dump(combined_history, f)
print('Training history saved as hybrid_efficientnet_history.json')
