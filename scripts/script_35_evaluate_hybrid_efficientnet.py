...
plt.plot(
    fpr,
    tpr,
    label=f"Hybrid EfficientNetB0 (AUC = {metrics['roc_auc']:.4f})")
...
def get_gradcam_heatmap(model, img_array, quality_val,
    ...
    try:
        ...
        for layer in model.layers:
            if hasattr(layer, 'layers'):
                try:
                    conv_layer_output = layer.get_layer(
...
for i in range(num_samples):
    ...
    axes[2].axis('off')
    plt.suptitle(
        f'Hybrid EfficientNetB0 Grad-CAM - {valid_image_names[i]}',
        fontsize=12)
    plt.tight_layout()
    # Save the generated plot to disk rather than displaying interactively
...
results_df = pd.DataFrame({'Image Name': valid_image_names,
                           ...
                           'quality_score': X_test_quality * 10})
