                ...
                if hasattr(layer, 'layers'):
                    for sub_layer in reversed(layer.layers):
                        if hasattr(sub_layer, 'output') and len(sub_layer.output.shape) == 4:
                            ...
                            grad_model = tf.keras.Model(inputs=model.input, outputs=[layer.get_layer(last_conv_layer_name).output, model.output])
                            break
                    if last_conv_layer_name:
                        break
    try:
        grad_model = tf.keras.Model(inputs=model.input, outputs=[model.get_layer(last_conv_layer_name).output, model.output])
    except ValueError:
        for layer in model.layers:
            if hasattr(layer, 'layers'):
                try:
                    conv_output = layer.get_layer(last_conv_layer_name).output
                    grad_model = tf.keras.Model(inputs=model.input, outputs=[conv_output, model.output])
                    break
                except ValueError:
                    continue
...
for i in range(num_samples):
    ...
    axes[2].axis('off')
    plt.suptitle(f'EfficientNetB0 Grad-CAM - {valid_image_names[i]}', fontsize=12)
    plt.tight_layout()
...
results_df.to_csv('efficientnet_predictions.csv', index=False)
