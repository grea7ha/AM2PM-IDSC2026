...
- **Phase 1 (Feature Extraction):** 10 epochs with frozen DenseNet121 base layers.
- **Phase 2 (Fine-Tuning):** 10 epochs unfreezing the top 20 layers with a lower learning rate (`1e-4`) for domain adaptation.
...
*(Note: The Hybrid EfficientNetB0 experienced feature collapse, failing to learn the minority class boundaries, evidenced by the 0.539 AUC.)*
...
**1. The Champion (Hybrid DenseNet121)**
...
**2. The Failed Model (Hybrid EfficientNetB0)**
...
**3. The Impact of Quality-Awareness (vs Baseline DenseNet121)**
...
**Limitations:** If a retinal image is completely ungradable—such as total blackout from a flash failure or a severe dense cataract entirely obscuring the retina—the model cannot conjure physiological data that does not exist. Furthermore, this network was exclusively trained on the Hillel Yaffe population. Its robustness across varying degrees of ocular pigmentation (fundus tessellation), diverse field camera hardware (domain shift), and differing ethnicities strictly requires multi-center clinical validation prior to deployment.
